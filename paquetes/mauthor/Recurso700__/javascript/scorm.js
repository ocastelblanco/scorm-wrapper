function getScorm() {
	...
	var patchCrossDomainError = function () {
		...
		if (initialDomain.indexOf(".") == -1) return true;
		...
		while (newDomain.indexOf(".") != -1 && wp != null && wp !== window) {
			try {
				...
				wp.document.getElementById("jeje");
				cdb = true;
				break;
			} catch (ex) {
	...
	scorm.getAPI = function (win) {
		if (API == null) {
			...
			if ((API == null) && (window.top != null)) {
				API = scanForAPI(window.top);
			}
			if ((API == null)) {
				API = scanForAPI(win)
			}
	...
	scorm.initializeScormCommunication = function (win) {
		console.log('mAuthor =>  patchCrossDomainError: ', patchCrossDomainError());
		this.getAPI(win);
